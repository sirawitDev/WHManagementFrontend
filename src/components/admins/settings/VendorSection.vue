...
const venderStore = useVendorStore()
const emit = defineEmits(['open-modal'])

const openAddModal = () => {
    emit('open-modal', 'vender')
}

const editVender = (vender) => {
    emit('open-modal', 'vender', vender)
}

const deleteVender = async (id, name) => {
    await venderStore.deleteVendor(id, name)
}
...
const viewFullDescription = (vendor) => {
    Swal.fire({
        title: vendor.name,
        html: `
            <div style="text-align: left; max-height: 400px; overflow-y: auto;">
                <p><b>Description:</b> ${vendor.description || '-'}</p>
                <p><b>Tel 1:</b> ${vendor.tel1 || '-'}</p>
                <p><b>Tel 2:</b> ${vendor.tel2 || '-'}</p>
                <p><b>Fax:</b> ${vendor.fax || '-'}</p>
                <p><b>Mobile Phone:</b> ${vendor.mobilePhone || '-'}</p>
                <p><b>Email:</b> ${vendor.email || '-'}</p>
                <p><b>Vendor Type:</b> ${vendor.venderType || '-'}</p>
...
const paginatedVender = computed(() => {
    const start = (currentPage.value - 1) * itemsPerPage
    const end = start + itemsPerPage
    return venderStore.vendors.slice(start, end)
})

const totalPages = computed(() => {
    return Math.ceil(venderStore.vendors.length / itemsPerPage)
})
            ...
            <div v-else class="mt-6 overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="vender in paginatedVender" :key="vender._id" class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500">
                                <div class="flex items-center gap-2">
                                    <span>{{ truncateDescription(vender.description) }}</span>
                                    <button v-if="isDescriptionLong(vender.description)" @click="viewFullDescription(vender)"
                            ...
                            <td class="px-4 py-3 text-right text-sm">
                                <button @click="editVender(vender)"
                                    class="text-[#14158C] hover:text-[#8A8CEB] mr-3">
                                    <Edit class="w-4 h-4 inline" />
                                </button>

                                <button @click="deleteVender(vender._id, vender.name)"
                        ...
                        <tr v-if="venderStore.vendors.length === 0">
                            <td colspan="4" class="px-4 py-6 text-center text-sm text-gray-500">
                                No vendors found. Click "ADD" to create one.
                            </td>
                        </tr>
                    </tbody>
                </table>
