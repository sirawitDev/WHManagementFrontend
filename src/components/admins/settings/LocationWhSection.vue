...
import { SquarePlus, MapPin, Edit, Trash2, Eye } from 'lucide-vue-next'
...
const openAddModal = () => {
    emit('open-modal', 'locationWh')
}

const editLocation = (location) => {
    emit('open-modal', 'locationWh', location)
}

const deleteLocation = async (id, name) => {
    await locationWhStore.deleteLocation(id, name)
}
...
const viewFullDescription = (description, locationName) => {
    if (!description || description === '-') {
        Swal.fire({
            icon: 'info',
            title: locationName,
            text: 'No description provided for this location.',
...
const paginatedLocations = computed(() => {
    ...
    return locationWhStore.locations.slice(start, end)
})

const totalPages = computed(() => {
    return Math.ceil(locationWhStore.locations.length / itemsPerPage)
})
...
<template>
    <div class="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 border border-gray-100">
        <div class="p-4 sm:p-5 lg:p-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-[#DDE0FF] rounded-lg">
                        <MapPin class="w-5 h-5 sm:w-6 sm:h-6 text-[#14158C]" />
                    </div>
                    <p class="text-lg sm:text-xl font-semibold text-gray-800">
                        Location Warehouse Settings
                    </p>
                </div>
            ...
            <!-- Loading State -->
            <div v-if="locationWhStore.loading" class="mt-6 text-center py-4">
            ...
            <div v-else class="mt-6 overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="location in paginatedLocations" :key="location._id"
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button 
                                        v-if="isDescriptionLong(location.description)"
                                        @click="viewFullDescription(location.description, location.name)"
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                <button @click="deleteLocation(location._id, location.name)"
                        ...
                        <tr v-if="locationWhStore.locations.length === 0">
                ...
                <!-- Pagination -->
                <div v-if="locationWhStore.locations.length > itemsPerPage" class="flex justify-end mt-4">
